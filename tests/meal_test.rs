...
mod tests {
    ...
    async fn test_meal_details_restaurant_with_people() {
        ...
        let meal_detail: MealDetail =
            serde_json::from_slice(&body).expect("Failed to deserialize meal detail");
        ...
        match meal_detail.food_source {
            Some(MealFoodSource::Restaurant {
                restaurant,
                meal_type,
            }) => {
    ...
    async fn test_get_meals_list() {
        ...
        let body = test::read_body(resp).await;
        let meals: Vec<xnote::models::meal::Meal> =
            serde_json::from_slice(&body).expect("Failed to deserialize meals list");
    ...
    async fn test_get_meal_by_id_restaurant() {
        ...
        let body = test::read_body(resp).await;
        let meal: xnote::models::meal::Meal =
            serde_json::from_slice(&body).expect("Failed to deserialize meal");
    ...
    async fn test_get_meal_by_id_not_found() {
        ...
        let req = test::TestRequest::get().uri("/meals/99999").to_request();
